...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function updateKPIs() {
    ...
    let limitsSummary = "";
    const todayExpensesByCategory = todayTransactions
  ...
  async function addOrUpdateTransaction(tx) {
    ...
    if (!db) {
      showToast("Database belum siap.", "warning");
      return;
    }
    try {
      if (tx.id) {
  ...
  function renderTransactionTable() {
    ...
    filteredTransactions
      ...
      .forEach((tx) => {
        const row = transactionRows.insertRow(); // ✔️ GANTI insertCell() JADI insertRow()
    ...
    myChart = new Chart(expensesChartCanvas, {
      ...
      data: {
        ...
        datasets: [
          {
            data: data,
            backgroundColor: ["#FF6384", "#36A2EB", "#FFCE56", "#4BC0C0", "#9966FF", "#FF9F40", "#8D6E63", "#C0CA33", "#795548", "#607D8B", "#E91E63", "#00BCD4", "#FFD700", "#ADFF2F"],
          },
        ],
      },
      options: {
  ...
  exportCsvBtn.addEventListener("click", () => {
    ...
    const rows = filtered.map((tx) => [tx.date, tx.type === "income" ? "Pemasukan" : "Pengeluaran", tx.category, tx.amount, `"${tx.description.replace(/"/g, '""')}"`]);
